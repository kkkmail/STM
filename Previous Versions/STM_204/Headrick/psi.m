...
(* 
This package implements the method described in the in the paper "Energy functionals for Calabi-Yau metrics" by M. Headrick and A. Nassar (arXiv:0908.2635), for numerically computing the Ricci-flat metric on a Calabi-Yau defined as a polynomial variety in $CP^N$, for the case of the polynomial

P = \sum_a(z^a)^{N+1} - (N+1) \psi \prod_a z^a ,
...
(* N1 is $N+1$. ps is $\psi$. qq is $\mathcal{Q}$. EE is $E$. H1 is the unnormalized version of $E$, i.e. the variance of v (numerator of equation (5.4) in the paper). *)

(* Before the package is loaded, the values of N1, k, and ps must be defined. *)
...
(* The symmetry group is generated by three factors: multiplication of the coordinates by N1'th roots of unity (subject to the constraint that the product of the roots be 1); permutations of the coordinates; and conjugation of the coordinates. A basis for the degree (k,k) homogeneous polynomials is given by the set of monomials that are invariant under the roots of unity (subject to the above constraint), symmetrized under permutations and conjugation. Each such monomial is determined by the matrix of its exponents, a 2 x N1 matrix of non-negative integers satisfying certain constraints. We begin with some functions for rearranging such matrices. *)

sortColumns[exp_List] := Transpose[ Sort[ Transpose[ exp ] ] ]

canonical[exp_List] := First[ Sort[ { sortColumns[exp], sortColumns[Reverse[exp]] } ] ]

RUsymmetric[exp_List] := Equal @@ Mod[ exp[[1]] - exp[[2]], N1 ]
...
onezeromat = { Table[ 1, {N1} ], Table[ 0, {N1} ] }

vec[exp_List] := 
  Sum[ whichkkpoly[ exp + SparseArray[{{1, a} -> N1}, {2, N1}] ], {a, N1} ] -
    N1 ps whichkkpoly [ exp + onezeromat ]
...
P = Total[HIC1^N1] - N1 ps Times @@ HIC
...
projectOntoP[u0_List]:= (
 w = Co[ (Q - N1 ( 1 - ps (Times @@ u0) ) Co[u0]) /. Thread[HIC -> u0] ];
 u1 = u0 + m w;
...
batch[eps_Real]:= (
  ...
  Pick[
    CPNpoints,
    Positive[ eps - Abs[ Total[CPNpoints^N1,{2}] - N1 ps (Times @@@ CPNpoints) + 1] ] ]
  )
...
(* the set of qq matrices for all the elements of RealPolynomials *)

kkqq = makeqq /@ kkpolynomials
...
(* gradient wrt h of the mean of v on our sample *)
...
(* Section 7: Minimizing the functional, using Levenbery-Marquardt, supplying the Jacobian *)
